...
STRIPE_SECRET_KEY = "sk_test_51LQXhFItMbzTFRwNFstoGvw3cAKWi1YBdT3rP2hPZY7cNREjewhxhnhUIc3gVIe7mf8LNqTn91FMKym71jVtLkPE00UrshMhDF"
...
class CustomerOrderCancelView(View):
    def dispatch(self, request, *args, **kwargs):
        if request.user.is_authenticated and Customer.objects.filter(user=request.user).exists():
    ...
    def get(self, request, pk, *args, **kwargs):
        try:
            order = Order.objects.get(id=pk, cart__customer=self.customer)
            context = {
                "order": order
            }
    ...
    def post(self, request, pk, *args, **kwargs):
        try:
            ...
            order.save()
            return redirect("ecomapp:customerorderdetail", pk=pk)
        except Exception as e:
            print(e)
            return redirect("ecomapp:customerprofile")
